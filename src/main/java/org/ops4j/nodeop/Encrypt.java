...
public class Encrypt extends BaseNodeOp<Encrypt>
{
  ...
  public Encrypt() throws OpsException
  {
    ...
    DEBUG("ENCRYPTING: ", config().getString("key"));
    byte[] key;
    String alg;
    ...
    try
    {
      key = Base64.getDecoder().decode(config().getString("key").trim());
      ...
      cipher.init(Cipher.ENCRYPT_MODE, k);
        ...
        | NoSuchPaddingException | IllegalArgumentException ex)
    {
      //WARN("Error initializing cipher: ",config().getString("key"), " - ", ex.getMessage());
